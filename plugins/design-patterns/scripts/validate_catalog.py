...
def validate_additional_provider_manifests() -> None:
    grok = load_json(PLUGIN / ".grok-plugin" / "plugin.json")
    kimi = load_json(PLUGIN / "kimi.plugin.json")
    for label, manifest in (("Grok", grok), ("Kimi", kimi)):
        require(manifest["name"] == "design-patterns", f"{label} plugin name must be canonical")
        require(manifest["version"] == "0.9.3", f"{label} plugin must match the immutable package release")
        require(manifest["skills"] == "./skills", f"{label} plugin must expose the shared skills")
        require(manifest["agents"] == "./agents", f"{label} plugin must expose the shared agents")
        require(manifest["commands"] == "./commands", f"{label} plugin must expose the shared commands")
        require(manifest["mcpServers"] == "./.portable-mcp.json", f"{label} plugin must expose the portable MCP config")
...
def main() -> int:
    ...
    validate_additional_provider_manifests()
